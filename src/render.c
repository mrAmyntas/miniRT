...
double	find_closest_object(t_scene *scene, t_ray *ray, int num[2], int set_cap)
{
	double	t[3];
	int		i;
	int		numb[3];
	...
	t[1] = find_hit_cy(scene, ray, &numb[1], set_cap);
	t[2] = find_hit_sphere(scene, ray, &numb[2]);
	i = find_smallest(t, 1, 3);
	if (i != -1)
	{
		num[0] = i;
		num[1] = numb[i];
...
void	light_strength2(t_scene *scene, t_variable x, t_vect3d Phit[2])
{
	...
	scene->light[scene->i].strength = x.diffuse;

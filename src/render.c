...
int	get_color(t_scene *scene, int num[2], t_vect3d Phit[2])
{
	...
	if (num[0] == PLANE)
		return (checkerboard_pl(scene, Phit[0], num[1]));
	...
	{
		if (scene->tor[num[1]].checker == 0)
			return (calculate_light(scene->tor[num[1]].hsl, scene));
		return (calculate_light(scene->tor[num[1]].lsh, scene));
	}
	return (-1);
}
...
void	set_pixel(t_data *data, t_scene *scene, int x, int y)
{
	...
	if (t > 0)
	{
		...
		if (isnan(color))
			color = 0xFFFFFFFF;
		// memset(data->mlx_img->pixels, color, data->mlx_img->width * data->mlx_img->height * sizeof(int));
